...
import { store, addTask, updateTask } from '../store.js';
...
// Opens the Quick Add modal pre-filled with an existing task's details for editing.
export function openEditTaskModal(task) {
  editingTaskId = task.id;
  form = {
    title: task.title || '',
    description: task.description || '',
    classification: task.classification || 'Academic',
    priority: task.priority || 'Medium',
    estimatedMinutes: task.estimatedMinutes || 30,
    scheduledDate: task.scheduledDate || getTodayDateString(),
    scheduledTime: task.scheduledTime || '',
    dueDate: task.dueDate || '',
    dueTime: task.dueTime || '',
    linkedSubjectId: task.linkedSubjectId || '',
    linkedGoalId: task.linkedGoalId || '',
    remindersEnabled: task.remindersEnabled !== undefined ? task.remindersEnabled : true,
    showAdvanced: !!(task.description || task.dueDate || task.dueTime || task.linkedGoalId || task.linkedSubjectId),
  };
  store.setState({ isQuickAddOpen: true });
}
    ...
    <div id="quick-add-modal-panel" class="w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
      <div class="p-4 sm:p-5 bg-gradient-to-r from-indigo-900 via-slate-900 to-indigo-950 text-white flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <div><h2 class="text-base font-bold">${editingTaskId ? 'Edit Task' : 'Quick Add Task'}</h2><p class="text-xs text-indigo-200">${editingTaskId ? 'Update the details of this task' : 'Capture actionable work with estimated duration & priority'}</p></div>
        </div>
      ...
      <form id="quick-add-form" class="p-4 sm:p-6 overflow-y-auto space-y-4 flex-1">
        ...
        <div class="pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
          ...
          <button type="submit" id="submit-quick-add-btn" class="inline-flex items-center gap-1.5 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 active:scale-98 text-white text-xs sm:text-sm font-semibold rounded-xl shadow-md transition">${editingTaskId ? icon('Check', 'w-4 h-4') : icon('Plus', 'w-4 h-4')}<span>${editingTaskId ? 'Save Changes' : 'Add to Schedule'}</span></button>
        </div>
      </form>
    </div>
  </div>`;
...
function wireEvents(root) {
  ...
  root.querySelector('#quick-add-form')?.addEventListener('submit', (e) => {
    ...
    if (editingTaskId) {
      ...
      addTask({ ...payload, status: 'Pending', customReminderMinutes: 5 });
    }
    closeModal();
  });
}
