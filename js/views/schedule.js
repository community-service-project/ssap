...
import { store, deleteFreeTimeSlot, addFreeTimeSlot, updateFreeTimeSlot, setTasksBulk, toggleTaskStatus } from '../store.js';
...
function renderFreeTimeManager(mountEl, selectedDate, freeSlots, tasks, userPreferences) {
          ...
          <div class="p-3.5 bg-white rounded-2xl border border-emerald-200/80 shadow-2xs flex flex-col justify-between gap-2.5">
            <div class="flex items-start justify-between gap-2">
              ...
              <div class="flex items-center gap-1 shrink-0">
                <button data-edit-slot="${slot.id}" title="Edit free slot" class="p-1.5 text-slate-400 hover:text-emerald-600 rounded-lg hover:bg-emerald-50 transition">${icon('Pencil', 'w-3.5 h-3.5')}</button>
  ...
  mountEl.querySelector('#add-free-slot-trigger-btn')?.addEventListener('click', () => { ui.isAddFreeSlotOpen = true; ui.editingSlotId = null; ui.slotForm = null; renderSchedule(document.getElementById('view-root')); });
  ...
  mountEl.querySelectorAll('[data-edit-slot]').forEach((b) => b.addEventListener('click', () => {
    const slot = store.state.freeTimeSlots.find((s) => s.id === b.getAttribute('data-edit-slot'));
    if (!slot) return;
    ui.editingSlotId = slot.id;
    ui.slotForm = { title: slot.title, startTime: slot.startTime, endTime: slot.endTime, date: slot.date || selectedDate, notes: slot.notes || '', isRecurring: !!slot.isRecurring, recurringDays: slot.recurringDays || [1, 2, 3, 4, 5] };
    ui.isAddFreeSlotOpen = true;
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
    <div class="w-full max-w-md bg-white rounded-3xl border border-slate-200 shadow-2xl p-6 relative">
      ...
      <div class="flex items-center gap-2 text-emerald-600 mb-1">
        ...
        <div><h2 class="text-lg font-black text-slate-900">${isEdit ? 'Edit Free Time Slot' : 'Add Free Time Slot'}</h2><p class="text-xs text-slate-500">Define periods when you are free to automatically arrange tasks.</p></div>
      </div>
      <form id="add-free-slot-form" class="mt-5 space-y-4">
        ...
        <div class="pt-2 flex items-center justify-end gap-2.5">
          ...
          <button type="submit" id="save-free-slot-btn" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow-md shadow-emerald-600/20 transition flex items-center gap-1.5">${isEdit ? icon('Pencil', 'w-4 h-4') : icon('Plus', 'w-4 h-4')}<span>${isEdit ? 'Save Changes' : 'Add Free Slot'}</span></button>
        </div>
      </form>
    </div>
  </div>`;
...
function wireEvents(root) {
  ...
  // Add free slot modal
  root.querySelector('#close-free-slot-modal-btn')?.addEventListener('click', () => { ui.isAddFreeSlotOpen = false; ui.editingSlotId = null; renderSchedule(root); });
  root.querySelector('#cancel-free-slot-btn')?.addEventListener('click', () => { ui.isAddFreeSlotOpen = false; ui.editingSlotId = null; renderSchedule(root); });
  ...
  root.querySelector('#add-free-slot-form')?.addEventListener('submit', (e) => {
    ...
    const payload = { title: f.title.trim() || 'Free Time Window', date: f.date, startTime: f.startTime, endTime: f.endTime, notes: f.notes.trim() || undefined, isRecurring: f.isRecurring, recurringDays: f.isRecurring ? f.recurringDays : undefined };
    if (ui.editingSlotId) {
      updateFreeTimeSlot(ui.editingSlotId, payload);
    } else {
      addFreeTimeSlot(payload);
    }
    ui.isAddFreeSlotOpen = false;
    ui.editingSlotId = null;
    ui.slotForm = null;
    renderSchedule(root);
  });
}
