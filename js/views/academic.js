...
import { store, addSubject, updateSubject, deleteSubject, addTimetableEntry, updateTimetableEntry, deleteTimetableEntry, addAcademicWork, updateAcademicWork, deleteAcademicWork, toggleAcademicWorkStatus } from '../store.js';
...
const ui = {
  ...
  editingSubjectId: null, editingTimetableId: null, editingWorkId: null,
        ...
        <div class="p-4 rounded-2xl border transition ${isCompleted ? 'bg-slate-50 border-slate-200 opacity-70' : 'bg-white border-slate-200/90 shadow-xs'}">
          <div class="flex items-start justify-between gap-3">
            ...
            <div class="flex items-center gap-1">
              <button data-edit-work="${work.id}" class="p-1.5 text-slate-400 hover:text-blue-600 rounded-lg hover:bg-slate-100 transition">${icon('Pencil', 'w-4 h-4')}</button>
            ...
            ${dayClasses.map((item) => {
              ...
              <div class="p-2 rounded-xl bg-blue-50/70 border border-blue-200/80 text-xs relative group">
                <div class="flex items-start justify-between">
                  <span class="font-bold text-blue-900">${sub?.code || 'Class'}</span>
                  <div class="flex items-center gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition">
                    <button data-edit-timetable="${item.id}" class="text-slate-400 hover:text-blue-600">${icon('Pencil', 'w-3 h-3')}</button>
                    <button data-delete-timetable="${item.id}" class="text-slate-400 hover:text-rose-600">${icon('X', 'w-3 h-3')}</button>
                  </div>
                </div>
      ...
      <div class="p-5 bg-white rounded-2xl border border-slate-200 shadow-xs flex flex-col justify-between">
        <div>
          <div class="flex items-start justify-between">
            ...
            <div class="flex items-center gap-1">
              <button data-edit-subject="${sub.id}" class="text-slate-400 hover:text-blue-600 p-1">${icon('Pencil', 'w-4 h-4')}</button>
...
function renderSubjectModal() {
  const s = ui.subj;
  const isEdit = !!ui.editingSubjectId;
  return modalShell(isEdit ? 'Edit Academic Subject' : 'Add Academic Subject', 'subject', `
    <form id="create-subject-form" class="space-y-3 text-xs sm:text-sm">
      ...
      <div class="pt-3 flex justify-end gap-2">
        ...
        <button type="submit" class="px-5 py-2 bg-blue-600 text-white font-bold rounded-xl shadow-xs">${isEdit ? 'Save Changes' : 'Save Subject'}</button>
      </div>
    </form>`);
}
...
function renderTimetableModal(subjects) {
  const t = ui.tt;
  const isEdit = !!ui.editingTimetableId;
  return modalShell(isEdit ? 'Edit Class Session' : 'Add Class Session', 'timetable', `
    <form id="create-timetable-form" class="space-y-3 text-xs sm:text-sm">
      ...
      <div class="pt-3 flex justify-end gap-2">
        ...
        <button type="submit" class="px-5 py-2 bg-blue-600 text-white font-bold rounded-xl shadow-xs">${isEdit ? 'Save Changes' : 'Add Class'}</button>
      </div>
    </form>`);
}
...
function renderWorkModal(subjects, goals) {
  const w = ui.work;
  const isEdit = !!ui.editingWorkId;
  return modalShell(isEdit ? 'Edit Assignment or Exam' : 'Add Assignment or Exam', 'work', `
    <form id="create-work-form" class="space-y-3 text-xs sm:text-sm">
      ...
      <div class="pt-3 flex justify-end gap-2">
        ...
        <button type="submit" class="px-5 py-2 bg-blue-600 text-white font-bold rounded-xl shadow-xs">${isEdit ? 'Save Changes' : 'Save Work'}</button>
      </div>
    </form>`);
}
...
function wireEvents(root) {
  ...
  root.querySelector('#add-subject-btn')?.addEventListener('click', () => {
    ui.editingSubjectId = null;
    ui.subj = { code: '', name: '', color: '#3B82F6', instructor: '', room: '' };
    ui.showSubjectModal = true; renderAcademic(root);
  });
  root.querySelector('#add-timetable-entry-btn')?.addEventListener('click', () => {
    ui.editingTimetableId = null;
    ui.tt = { subjectId: '', dayOfWeek: 1, startTime: '09:00', endTime: '10:15', type: 'Lecture', room: '' };
    if (store.state.subjects.length > 0) ui.tt.subjectId = store.state.subjects[0].id;
    ui.showTimetableModal = true; renderAcademic(root);
  });
  root.querySelector('#add-academic-work-btn')?.addEventListener('click', () => {
    ui.editingWorkId = null;
    ui.work = { title: '', subjectId: '', type: 'Assignment', dueDate: '', dueTime: '23:59', estimatedMinutes: 60, priority: 'High', goalId: '' };
    if (store.state.subjects.length > 0) ui.work.subjectId = store.state.subjects[0].id;
    ui.showWorkModal = true; renderAcademic(root);
  });
  root.querySelectorAll('[data-edit-work]').forEach((b) => b.addEventListener('click', () => {
    const work = store.state.academicWorks.find((w) => w.id === b.getAttribute('data-edit-work'));
    if (!work) return;
    ui.editingWorkId = work.id;
    ui.work = { title: work.title, subjectId: work.subjectId, type: work.type, dueDate: work.dueDate, dueTime: work.dueTime || '23:59', estimatedMinutes: work.estimatedMinutes, priority: work.priority, goalId: work.linkedGoalId || '' };
    ui.showWorkModal = true; renderAcademic(root);
  }));
  root.querySelectorAll('[data-edit-subject]').forEach((b) => b.addEventListener('click', () => {
    const sub = store.state.subjects.find((s) => s.id === b.getAttribute('data-edit-subject'));
    if (!sub) return;
    ui.editingSubjectId = sub.id;
    ui.subj = { code: sub.code, name: sub.name, color: sub.color, instructor: sub.instructor || '', room: sub.room || '' };
    ui.showSubjectModal = true; renderAcademic(root);
  }));
  root.querySelectorAll('[data-edit-timetable]').forEach((b) => b.addEventListener('click', () => {
    const entry = store.state.timetable.find((t) => t.id === b.getAttribute('data-edit-timetable'));
    if (!entry) return;
    ui.editingTimetableId = entry.id;
    ui.tt = { subjectId: entry.subjectId, dayOfWeek: entry.dayOfWeek, startTime: entry.startTime, endTime: entry.endTime, type: entry.type || 'Lecture', room: entry.room || '' };
    ui.showTimetableModal = true; renderAcademic(root);
  }));
  root.querySelectorAll('[data-close-modal]').forEach((b) => b.addEventListener('click', () => {
    const which = b.getAttribute('data-close-modal');
    if (which === 'subject') { ui.showSubjectModal = false; ui.editingSubjectId = null; }
    if (which === 'timetable') { ui.showTimetableModal = false; ui.editingTimetableId = null; }
    if (which === 'work') { ui.showWorkModal = false; ui.editingWorkId = null; }
    renderAcademic(root);
  }));
  ...
  root.querySelector('#create-subject-form')?.addEventListener('submit', (e) => {
    ...
    const payload = { code: ui.subj.code.trim().toUpperCase(), name: ui.subj.name.trim(), color: ui.subj.color, instructor: ui.subj.instructor.trim() || undefined, room: ui.subj.room.trim() || undefined };
    if (ui.editingSubjectId) {
      updateSubject(ui.editingSubjectId, payload);
    } else {
      addSubject(payload);
    }
    ui.subj = { code: '', name: '', color: '#3B82F6', instructor: '', room: '' };
  ...
  root.querySelector('#create-timetable-form')?.addEventListener('submit', (e) => {
    ...
    const payload = { subjectId: ui.tt.subjectId, dayOfWeek: Number(ui.tt.dayOfWeek), startTime: ui.tt.startTime, endTime: ui.tt.endTime, type: ui.tt.type, room: ui.tt.room.trim() || undefined };
    if (ui.editingTimetableId) {
      updateTimetableEntry(ui.editingTimetableId, payload);
    } else {
  ...
  root.querySelector('#create-work-form')?.addEventListener('submit', (e) => {
    ...
    const payload = { subjectId: ui.work.subjectId, title: ui.work.title.trim(), type: ui.work.type, dueDate: ui.work.dueDate, dueTime: ui.work.dueTime || undefined, estimatedMinutes: ui.work.estimatedMinutes, priority: ui.work.priority, linkedGoalId: ui.work.goalId || undefined };
    if (ui.editingWorkId) {
      updateAcademicWork(ui.editingWorkId, payload);
    } else {
      addAcademicWork({ ...payload, status: 'Pending' });
    }
    ui.work = { title: '', subjectId: '', type: 'Assignment', dueDate: '', dueTime: '23:59', estimatedMinutes: 60, priority: 'High', goalId: '' };
    ui.editingWorkId = null;
