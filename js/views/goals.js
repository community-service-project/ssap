...
import { store, addGoal, deleteGoal, toggleMilestone, addMilestone, toggleAtRisk } from '../store.js';
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-fade-in">
    <div class="w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-slate-200 p-5 sm:p-6 overflow-y-auto max-h-[90vh]">
      <div class="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
        <h3 class="text-base font-bold text-slate-900">Define New Goal</h3>
      ...
      <form id="create-goal-form" class="space-y-4 text-xs sm:text-sm">
        ...
        <div class="pt-4 flex justify-end gap-2 border-t border-slate-100">
          ...
          <button type="submit" class="px-5 py-2 bg-amber-500 text-white font-bold rounded-xl shadow-xs">Save Goal</button>
        </div>
      </form>
    </div>
  </div>`;
...
function wireEvents(root) {
  root.querySelector('#open-add-goal-modal-btn')?.addEventListener('click', () => { ui.showAddGoalModal = true; renderGoals(root); });
  root.querySelector('#close-add-goal-modal-btn')?.addEventListener('click', () => { ui.showAddGoalModal = false; renderGoals(root); });
  root.querySelector('#cancel-add-goal-btn')?.addEventListener('click', () => { ui.showAddGoalModal = false; renderGoals(root); });
