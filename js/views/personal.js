...
import { store, addPersonalWork, deletePersonalWork, updatePersonalWork, updateSleepRoutine } from '../store.js';
import { DAYS_OF_WEEK, formatMinutes, formatTimeString } from '../time.js';

function freshPersonalForm() {
  return { title: '', category: 'Exercise', estimatedMinutes: 45, startTime: '07:30', priority: 'Medium', isRecurring: true, recurringDays: [1, 3, 5] };
}
          ...
          <div>
            <div class="flex items-start justify-between">
              ...
              <div class="flex items-center gap-1">
                <button data-edit-personal="${work.id}" class="p-1 text-slate-400 hover:text-indigo-600 transition" title="Edit routine">${icon('Pencil', 'w-4 h-4')}</button>
                <button data-delete-personal="${work.id}" class="p-1 text-slate-400 hover:text-rose-600 transition" title="Delete routine">${icon('Trash2', 'w-4 h-4')}</button>
              </div>
            </div>
...
function renderAddModal() {
  const f = ui.form;
  const isEditing = !!ui.editingWorkId;
  return `
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-fade-in">
    <div class="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 p-5 sm:p-6">
      <div class="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
        <h3 class="text-base font-bold text-slate-900">${isEditing ? 'Edit Personal Activity / Routine' : 'Add Personal Activity / Routine'}</h3>
      ...
      <form id="add-personal-form" class="space-y-3 text-xs sm:text-sm">
        ...
        <div class="pt-4 flex justify-end gap-2">
          ...
          <button type="submit" class="px-5 py-2 bg-purple-600 text-white font-bold rounded-xl shadow-xs">${isEditing ? 'Save Changes' : 'Add Routine'}</button>
        </div>
      </form>
    </div>
  </div>`;
...
function wireEvents(root) {
  ...
  root.querySelector('#open-add-personal-modal-btn')?.addEventListener('click', () => {
    ui.editingWorkId = null;
    ui.form = freshPersonalForm();
    ui.showAddModal = true;
    renderPersonal(root);
  });
  root.querySelector('#close-add-personal-modal-btn')?.addEventListener('click', () => { ui.showAddModal = false; ui.editingWorkId = null; renderPersonal(root); });
  root.querySelector('#cancel-add-personal-btn')?.addEventListener('click', () => { ui.showAddModal = false; ui.editingWorkId = null; renderPersonal(root); });

  root.querySelectorAll('[data-edit-personal]').forEach((b) => b.addEventListener('click', () => {
    const work = store.state.personalWorks.find((p) => p.id === b.getAttribute('data-edit-personal'));
    if (!work) return;
    ui.editingWorkId = work.id;
    ui.form = {
      title: work.title || '',
      category: work.category || 'Exercise',
      estimatedMinutes: work.estimatedMinutes || 45,
      startTime: work.startTime || '',
      priority: work.priority || 'Medium',
      isRecurring: !!work.isRecurring,
      recurringDays: work.recurringDays || [1, 3, 5],
    };
    ui.showAddModal = true;
    renderPersonal(root);
  }));
  ...
  root.querySelector('#add-personal-form')?.addEventListener('submit', (e) => {
    ...
    const payload = {
      ...
      recurringDays: f.isRecurring ? f.recurringDays : undefined,
    };
    if (ui.editingWorkId) {
      updatePersonalWork(ui.editingWorkId, payload);
    } else {
      addPersonalWork({ ...payload, status: 'Scheduled', createdAt: new Date().toISOString() });
    }
    ui.editingWorkId = null;
    ui.form = freshPersonalForm();
