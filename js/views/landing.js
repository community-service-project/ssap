...
import { signInDemo } from '../auth.js';
    ...
    <section class="relative pt-12 pb-20 sm:pt-20 sm:pb-28 bg-gradient-to-b from-slate-950 via-slate-900 to-indigo-950 text-white overflow-hidden">
      ...
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        <div class="mt-8 sm:mt-10 flex flex-col sm:flex-row items-center justify-center gap-3.5">
          ...
          <button id="hero-demo-account-btn" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-xl bg-cyan-950/80 hover:bg-cyan-900 text-cyan-300 border border-cyan-700/50 font-semibold text-sm transition">${icon('Compass', 'w-4 h-4 text-cyan-400')}<span>Explore Demo Student</span></button>
    ...
    <footer class="bg-slate-950 text-slate-400 py-10 border-t border-slate-800 text-xs">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-4 text-xs font-medium">
          ...
          <button id="footer-demo-link" class="text-indigo-400 hover:text-indigo-300">Demo Student</button>
        </div>
      </div>
    </footer>
  </div>`;
...
async function useDemo() {
  try {
    ...
    alert(err.message || 'Demo sign-in failed. Make sure Anonymous Sign-ins are enabled in your Supabase project.');
  }
}
...
function wireEvents(root) {
  ...
  root.querySelector('#hero-demo-account-btn')?.addEventListener('click', useDemo);
  ...
  root.querySelector('#footer-demo-link')?.addEventListener('click', useDemo);
}
