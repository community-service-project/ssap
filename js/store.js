...
import {
  ...
  INITIAL_SUBJECTS,
  INITIAL_TIMETABLE,
  INITIAL_ACADEMIC_WORKS,
  INITIAL_PERSONAL_WORKS,
  INITIAL_GOALS,
  INITIAL_TASKS,
  INITIAL_FREE_TIME_SLOTS,
} from './initialData.js';
...
export async function loadAllDataForUser() {
  ...
  // First-time login: no rows anywhere yet -> seed demo data so the app isn't empty
  const isBrandNew =
    subjects.length === 0 && (tasksRes.data || []).length === 0 && goals.length === 0 && (personalRes.data || []).length === 0;
...
async function seedDemoData() {
  ...
  if (!userId) return;
  await Promise.all([
    supabase.from('academic_subjects').upsert(INITIAL_SUBJECTS.map(subjectToRow)),
    supabase.from('tasks').upsert(INITIAL_TASKS.map(taskToRow)),
    supabase.from('personal_works').upsert(INITIAL_PERSONAL_WORKS.map(personalWorkToRow)),
    supabase.from('free_time_slots').upsert(INITIAL_FREE_TIME_SLOTS.map(freeSlotToRow)),
  ]);
  await supabase.from('timetable_entries').upsert(INITIAL_TIMETABLE.map(timetableToRow));
  await supabase.from('academic_works').upsert(INITIAL_ACADEMIC_WORKS.map(academicWorkToRow));
  for (const g of INITIAL_GOALS) {
    await supabase.from('goals').upsert(goalToRow(g));
    await supabase.from('goal_milestones').upsert(g.milestones.map((m) => milestoneToRow(m, g.id)));
  }
...
export async function resetDemoData() {
  ...
  await Promise.all([
    ...
    supabase.from('academic_subjects').delete().eq('user_id', userId),
    supabase.from('personal_works').delete().eq('user_id', userId),
    supabase.from('goals').delete().eq('user_id', userId),
    supabase.from('free_time_slots').delete().eq('user_id', userId),
  ]);
  await seedDemoData();
  const resetNotif = {
    id: `notif-reset-${Date.now()}`, title: 'Demo Data Restored',
    message: 'All modules have been refreshed with complete student sample data.',
    type: 'system', timestamp: new Date().toISOString(), read: false,
  };
  store.setState({ notifications: [resetNotif, ...store.state.notifications] });
  upsert('notifications', notifToRow(resetNotif));
}
