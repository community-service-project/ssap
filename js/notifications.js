 ...
 *  Mobile browsers (Chrome/Firefox on Android, etc.) do not allow the
 *  `new Notification()` constructor to be called from page JS — it throws
 *  "Illegal constructor" and the notification silently never appears.
 *  They require notifications to be shown via
 *  ServiceWorkerRegistration.showNotification() instead, which desktop
 *  browsers also support. So we always prefer the service worker path and
 *  only fall back to the constructor on the (rare) browser that has
 *  Notification support but no service worker registration. */
export async function sendSmartNotification(title, options) {
  ...
  const notifOptions = {
    body: options.body,
    icon: options.icon || '/public/pwa-192x192.png',
    badge: options.badge || '/public/pwa-192x192.png',
